...
import { LiquidFileUploader, UploadTheFiles } from './LiquidFileUploader';
...
async function run() {
    try {
        let LiquidUploader: LiquidFileUploader = new LiquidFileUploader(tl.getInput('url', true), tl.getInput('token', true),
            ...
            tl.getInput('email', true), tl.getInput('subject', true), tl.getInput('body', true), tl.getBoolInput("group", true),
            tl.getBoolInput("dell", true), tl.getBoolInput("auth", true)
        );
